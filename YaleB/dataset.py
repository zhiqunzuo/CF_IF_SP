...
class ExtendedYaleBDataLoader(PrivacyDataLoader):
    ...
    def load(self):
        ...
        sorted_test_data = []
        sorted_test_label = []
        sorted_test_sensitive_label = []
        for person in range(38):
            person_index = test_label == person
            person_data = test_data[person_index]
            person_label = test_label[person_index]
            person_sensitive_label = test_sensitive_label[person_index]
            
            sort_index = torch.argsort(torch.from_numpy(person_sensitive_label).long())
            person_data = person_data[sort_index]
            person_label = person_label[sort_index]
            person_sensitive_label = person_sensitive_label[sort_index]
            
            index = [0, 2, 5, 7, 10, 1, 3, 6, 8, 11]
            sorted_test_data.extend(person_data[index])
            sorted_test_label.extend(person_label[index])
            sorted_test_sensitive_label.extend(person_sensitive_label[index])
        test_data = np.array(sorted_test_data)
        test_label = np.array(sorted_test_label)
        test_sensitive_label = np.array(sorted_test_sensitive_label)
...
class CFYaleBDataLoader(PrivacyDataLoader):
    def __init__(self, data_path, train_data=None, train_label=None, train_sensitive_label=None, test_data=None, test_label=None,
                 test_sensitive_label=None, trainset=None, testset=None, args=None, device=None):
        self.name = "yale"
        self.data_path = data_path
