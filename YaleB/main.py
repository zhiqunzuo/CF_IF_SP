...
def evaluation(args, device, eval_model, model=None, clf_xy=None, is_e2e=True, trainset=None, testset=None):
    ...
    if is_e2e:
        ...
        y_pred, y_acc, s_acc, s_pred, cf = z_evaluator(args, model, clf_xy, device, eval_model, 'ours', trainset, testset)
    else:
        model = FarconVAE(args, device)
...
def e2e(args, device):
    ...
    for it_seed in range(730, 730+args.n_seed):
        ...
        if args.data_name == 'yaleb':
            y_acc, s_acc, cf = evaluation(args, device, eval_model, model, clf_xy, is_e2e=True, trainset=data, testset=data)
        else:
