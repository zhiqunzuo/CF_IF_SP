...
class FarconVAELoss(nn.Module):
    def __init__(self, args, device, total_train_it):
        ...
        self.recon_race_lossf = nn.CrossEntropyLoss()
        self.recon_x_lossf = nn.MSELoss()
        self.recon_s_lossf = nn.BCELoss()
        self.pred_lossf = nn.MSELoss()

    def forward(self, out1, out2, x_ori, x_cont, s_ori, s_cont, y, model, current_iter, is_train=True):
        if is_train:
            # 1.1. ELBO - Reconstruction Loss
            recon_x_loss1 = 0.5 * self.recon_race_lossf(out1[1][0][:, 2:], x_ori[:, 2:]) + 0.5 * self.recon_x_lossf(out1[1][0][:, :2], x_ori[:, :2])
            recon_x_loss2 = 0.5 * self.recon_race_lossf(out2[1][0][:, 2:], x_cont[:, 2:]) + 0.5 * self.recon_x_lossf(out2[1][0][:, :2], x_cont[:, :2])
            recon_s_loss1, recon_s_loss2 = self.recon_s_lossf(out1[1][1], s_ori.float()), self.recon_s_lossf(out2[1][1], s_cont.float())
            ...
            if self.args.gamma != 0:
                ...
                ms_ori = self.recon_race_lossf(x_mix_rec1[:, 2:], x_ori[:, 2:]) + self.recon_x_lossf(x_mix_rec1[:, :2], x_ori[:, :2]) + \
                    self.recon_s_lossf(s_mix_rec1, s_ori.float())
                ms_cont = self.recon_race_lossf(x_mix_rec2[:, 2:], x_cont[:, 2:]) + self.recon_x_lossf(x_mix_rec2[:, :2], x_cont[:, :2]) + \
                    self.recon_s_lossf(s_mix_rec2, s_cont.float())
            ...
            # recon loss
            recon_x_loss = 0.5 * self.recon_race_lossf(out1[1][0][:, 2:], x_ori[:, 2:]) + \
                0.5 * self.recon_x_lossf(out1[1][0][:, :2], x_ori[:, :2])
            recon_s_loss = self.recon_s_lossf(out1[1][1], s_ori.float())
...
class Criterion(nn.Module):
    ...
    def forward(self, inputs, target, current_step):
        ...
        # entropy loss
        Loss_e = entropy_loss(zt_s, self.args.kld_agg)
